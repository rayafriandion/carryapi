...
2. **自定义模型**(模型管理 → 模型):建一个对外模型名(如 `my-gpt4`),映射到某供应商的真实模型(如 `gpt-4o`)。也可点**「从供应商导入」**省去手填:选择供应商后拉取其上游模型列表(已有同名模型会标「已存在」并禁用),勾选后「导入」批量创建;导入的模型以**禁用态草稿(enabled=0)**创建,同名自动跳过。导入后仍需在下一步配置价格并在模型中启用,模型才可被调用。
