...
1. **上游供应商**(模型管理 → 供应商):添加真实上游,填 `名称`、`base_url`、`API Key`、`协议`(openai_chat / openai_responses / anthropic)。每行有**「测试」**按钮,点击即调 `POST /api/providers/{id}/test` 检查该供应商连通性并显示延迟(如「可用 123ms」或「不可用: …」),便于确认配置正确。
2. **自定义模型**(模型管理 → 模型):建一个对外模型名(如 `my-gpt4`)。也可点**「从供应商导入」**省去手填:选择供应商后拉取其上游模型列表(已有同名模型会标「已存在」并禁用),勾选后「导入」批量创建;导入的模型以**禁用态草稿(enabled=0)**创建,同名自动跳过。导入后仍需在下一步配置价格并在模型中启用,模型才可被调用。
3. **定价**(模型管理 → 定价):给模型设输入/输出价格,可选缓存读/写价格(每百万 token)。调价只影响后续请求,旧账单不变。
4. **路由配置**(管理后台 → 路由配置):这是多供应商多模型路由的**唯一配置入口**。以模型为主视图,每个模型可展开管理多条**上游绑定(binding)**——每条绑定指定一个供应商 + 上游模型名 + 优先级(priority,越小越优先) + 权重(weight,同优先级内加权随机) + 启用状态。一个自定义模型可挂多条绑定,路由时按策略在绑定间选择:
   - **自动路由 + 优先级**:取最高优先级组,组内按权重加权随机。
   - **自动路由 + 故障转移**:按优先级顺序逐个尝试,遇 429/5xx 自动切换下一个绑定。
   - **自动路由 + 健康感知**:跳过不健康的绑定(成功率 < 80%),其余按优先级/权重选。
   - **随机**:在所有启用的绑定间按权重加权随机。
   - 路由时发给上游的 `model` 字段会被替换为绑定里指定的上游模型名(如客户端发 `my-gpt4`,实际发给 OpenAI 的是 `gpt-4o`),避免上游拒绝下游自定义名。
...
> **API base_url**:仪表盘首页会展示网关自身的 OpenAI 兼容端点地址(形如 `http://<host>:<port>/v1`,可一键复制),即上文 curl 示例中的 `http://localhost:8067/v1` 部分,客户端直接用它作为 base URL。广播开启(监听 `0.0.0.0`)时按请求 Host 生成,否则为 `127.0.0.1:<port>`。
...
路由配置页(管理后台 → 路由配置)为每个上游绑定(binding)展示**基于真实转发日志**的健康状态和性能数据,不模拟不预估:

- **6 格状态时间轴**(覆盖最近 24 小时,每格 4 小时),按该时段内请求成功率分级:
  - 🟢 绿(正常):成功率 ≥ 95%
  - 🟡 黄(警告):成功率 80% ~ 95%
  - 🔴 红(异常):成功率 < 80%
  - ⚪ 灰(无数据):该时段无请求流量
- **客户端主动断开**的请求(status_code=200 但流中途断)不计入失败统计,既不算成功也不算失败。
- 点击某绑定的详情可查看 24 小时性能指标:**平均响应时延**(duration_ms)、**平均首字延迟 TTFT**(从请求开始到上游首字节到达)、**吞吐量**(每小时请求数)。这些指标不影响时间轴颜色,仅作运维参考。
- 健康状态由后台每 1 分钟预计算一次(从 `request_logs` 聚合),路由器读缓存不查数据库,有最多 1 分钟延迟。
